...
for (var i = 0; i < icon.length; i++) {
	...
	var ins = document.createElement("div"); //creating new div
	ins.id = 'b' + i;
	ins.setAttribute("class", "song");
	var main_div = document.getElementById("sound_card"); //getting the div in which we have to place new div
	//ins.onclick=function(){
	//next(this);
	//};
	main_div.appendChild(ins);
...
// For setting up the in which mod the music will play
...
async function getTime() {
	...
	if (value == null) {
		rand_play();
		window.alert("Failed to detect Face...");
	}
	else if (value == "angry") {
		window.alert(value);
		moody(0);
